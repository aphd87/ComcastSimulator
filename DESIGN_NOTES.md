...
11. **Decisions-flow redesign: both games are now a single scrolling page** — the request from earlier this session ("students should have prompts that force them to make decisions as they scroll down... then click simulate at the bottom"), previously deferred as its own design pass.
    ...
    - **Still the single most important gap**: no real human browser click-through has ever happened, on Day 1 or Day 2 (or any feature built since) — the two bugs just found are proof a human pass would very likely turn up more.

**Still open, carried over from 2026-07-24 below:**
- No real human browser click-through has ever happened, on Day 1 or Day 2 (or the newer tabs/features/timeline reshape/concept-type work added since) — this is now the single most important gap.
