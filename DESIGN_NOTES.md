...
8. **Overall leaderboard, Greenlighting → real roster wiring, AI pitch generator** — three more items from the same follow-up conversation. 90 tests passing.
...
**In progress, not yet complete** (see the running task list): Level Notables (best year, most improved, consistency, genre-diversity trend, shows-greenlit) computed at Submit time and shown both to the student (a plain-language summary at Complete) and on the Leaderboard as new badges, plus a live Attempts-Used display. Genre needs to be added to each `yearly_log` show-row first (not currently tracked per year) so diversity trend can be computed retroactively without depending on the live, possibly-changed roster.
