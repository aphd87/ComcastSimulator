...
9. **Level Notables** — per-team plain-language playthrough highlights, computed once at Submit time from `yearly_log` (not the live roster, which cancellations/new-greenlights can have since changed). `compute_level_notables()` (`utils/game_state.py`) returns `best_year` (highest-margin year), `most_improved` (last year's margin minus first year's), `consistency_score` (0-100, derived from margin stdev across the level), and `diversity_trend` (first-year genre HHI minus last-year — negative means the slate diversified; `None` if a year had no active shows to measure). Depends on last session's genre-per-show-row tracking in `_compute_year()`. Stored as `record_attempt()`'s new `notables` field (own top-level key, same reasoning as `slate_summary` — `details` is iterated wholesale by the Leaderboard's badge rendering). **Movies-track equivalent**: `compute_movie_notables()`, wired into `pages/movies.py`'s Submit handler — `best_cycle`, `most_improved`, `consistency_score` map directly, but `genre_variety` (count of distinct genres across the 3 cycles) stands in for `diversity_trend` since a single movie-cycle is always one genre (HHI trivially 1.0, so a per-cycle *trend* isn't a meaningful signal the way it is for the TV side's larger annual rosters). 109 tests passing.
   - **Still open**: the student-facing summary panel at Complete and the Leaderboard Attempts-Used/Notables badges are not yet built — `notables` is computed and stored but nothing renders it yet.
