...
1. Award-season/critical-reception as its own dimension beyond genre-driven variance width — e.g. a specific "buzz" input separate from star power. Not started; current genre-variance work covers the more load-bearing gap.
2. The actual interactive click-through (Release → Results → Complete → Submit) is still only verified by direct unit tests on the underlying functions (`utils/movie_models.py`, `utils/game_state.py`'s leaderboard path) plus a manual code read — not by driving the real Streamlit widgets, for the AppTest reason above. A human browser pass is still the one thing nobody has done.
