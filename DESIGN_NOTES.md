...
- **New request, 2026-07-27 (not yet scoped or built): redesign the Decisions flow (both TV/Streaming's 4-step wizard in `pages/simulation.py` and Movies' 3-phase click-through in `pages/movies.py`) into a single scrolling page** — user's words: "students should have prompts that force them to make decisions as they scroll down... then click simulate at the bottom." This is a real UX overhaul, not a copy tweak — needs its own design pass (how much of the current Back/Next step structure survives as scroll sections vs. a genuine rewrite) before touching either file. Explicitly deferred rather than bundled into the concept-type work above to avoid touching the same UI mid-change twice.
