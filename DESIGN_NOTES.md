...
2. No tests yet for `pages/movies.py`'s state machine itself (phase transitions, session-state handling) — `utils/movie_models.py`'s pure functions are covered, the Streamlit page logic isn't. Matches Day 1's existing convention (no page-layer tests there either), but worth reconsidering.
