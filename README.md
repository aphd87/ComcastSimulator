...
**`FREE_NAVIGATION`** (added 2026-08-07, **default flipped to on 2026-08-24**) — lets every team open Bravo or Peacock directly from the Active Network selector, without first passing (or retrying) the prior level. Defaults to **on**: this tool is instructor-directed classroom use, not self-paced mastery progression, so the old sequential gate (pass, or fail-and-retry-once, to advance) mostly just added friction — a team couldn't explore Bravo/Peacock's mechanics early, or recover from a bad Oxygen run without a forced replay. All three networks' show slates are seeded for every team at registration regardless of play order, so jumping ahead is mechanically safe. Attempt history, official scores, and leaderboard rankings are unaffected — this only changes which levels are clickable, not how they're scored.

An instructor who specifically wants the old sequential-gating behavior can opt back into it per deployment:

```toml
FREE_NAVIGATION = false
```

Same read-once-at-startup caveat as `YEARS_PER_LEVEL`: restart (or wait for the next redeploy) after changing it.
