...
def _render_board_tab(team: str, net: str, info: dict):
    ...
    (NETWORK_ORDER) and the Day 2 Movies leaderboard, which use the same
    FERPA-safe get_network_leaderboard()/load_leaderboard() infra."""
...
def render():
    ...
    for tab, net in zip(net_tabs, all_nets):
        with tab:
            _render_board_tab(team, net, all_infos[net])
